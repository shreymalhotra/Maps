...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    coordinates = [restaurant_location(point) for point in cluster]
    return [mean([lat_long[0] for lat_long in coordinates]), mean([lat_long[1] for lat_long in coordinates])]
    # END Question 5
...
def k_means(restaurants, k, max_updates=100):
    ...
    old_centroids, n = [], 0
    # Select initial centroids randomly by choosing k different restaurantse
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(centroid) for centroid in clusters]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared = sxy/sxx, mean2-sxy/sxx*mean1, pow(sxy, 2)/(sxx*syy)
    # END Question 7
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in user_reviewed_restaurants(user, restaurants):
            dictionary_of_ratings[r] = user_rating(user,r )
        else:
            dictionary_of_ratings[r] = predictor(restaurant)
